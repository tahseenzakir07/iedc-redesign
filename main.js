const content = {
    ...
    about: {
        ...
        objectives: [
            ...
            { title: "Provide Entrepreneurial Support", description: "Support other institutions around RSET to mold and effectively carry out entrepreneurial activities in their campus.", icon: "🤝" },
            { title: "Increase Representation of Women", description: "Encourage and facilitate the involvement of women in entrepreneurial endeavors.", icon: "👩‍💻" },
            { title: "More Startups", description: "Promote start-up initiatives from Faculty and Students.", icon: "📈" },
            { title: "Build Individuals", description: "Prepare students to finish their engineering studies equipped with all the entrepreneurial skills necessary to thrive in their future pursuits.", icon: "🛠️" }
        ]
    },
    gallery: [
    ...
    if (grid) {
        grid.innerHTML = content.reports.items.map(r => `
            <section class="bento-card span-6 reveal">
                ...
                <a href="${r.url}" target="_blank" class="gradient-text" style="display: block; margin-top: 2rem; font-weight: 800; text-decoration: none;">DOWNLOAD →</a>
            </section>
        `).join('');
    }
}
